...
**StoplossGuard** (global+pair), **CooldownPeriod** (pair), **LowProfitPairs** (pair). Platform additions: **MaxDailyLoss**, **MaxWeeklyLoss** (Phase 4, calendar-period loss budgets) and **ConsecutiveLossGuard** (Phase 5, losing-streak circuit breaker) in `freqtrade/plugins/protections/`.
